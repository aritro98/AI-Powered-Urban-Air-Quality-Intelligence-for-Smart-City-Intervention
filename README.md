...
- A **FastAPI backend**
- A **6-agent orchestration layer**
- A **browser-based dashboard**
- **Real public API integrations**
- **Forecasting and dispersion logic**
- **Validation and response-time benchmarking**
- Optional **LLM-assisted reasoning** when an Anthropic key is configured
...
Forecasting uses:
- Open-Meteo Air Quality forecast data
...
- source confidence
- exposure proxy
- dispersion modelling output
- proximity to a plausible upwind target
...
This makes the system suitable for judging not only accuracy, but also operational latency.
